const listDailyMetricsService = require('../../services/metrics/listDailyMetricsService');

async function listDailyMetricsController(req, res, next) {
  try {
    const user = req.user;
    ...
    if (user.role === 'ADMIN' && !clientId && user.managedClientIds.length > 0) {
      const filteredMetrics = metrics.filter((metric) =>
        user.managedClientIds.some((id) => id.toString() === metric.clientId._id.toString())
      );
...
module.exports = listDailyMetricsController;
