const listCustomMetricFieldsService = require('../../services/customMetricField/listCustomMetricFieldsService');

async function listCustomMetricFieldsController(req, res, next) {
  try {
    const user = req.user;
    ...
    const fields = await listCustomMetricFieldsService(filters);
    ...
    if (user.role === 'ADMIN' && !clientId && user.managedClientIds.length > 0) {
      const filteredFields = fields.filter((field) =>
        user.managedClientIds.some((id) => id.toString() === field.clientId.toString())
      );
      return res.status(200).json({
        success: true,
        message: 'Custom metric fields retrieved successfully',
        data: filteredFields,
      });
    }
...
module.exports = listCustomMetricFieldsController;
